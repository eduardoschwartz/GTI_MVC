...
namespace GTI_Dal.Classes {
    public class Imovel_Data {
            ...
            using (GTI_Context db = new GTI_Context(_connection)) {
                ...
                foreach (var query in reg) {
                    ...
                    if (!string.IsNullOrEmpty(query.Cpf) && query.Cpf.ToString().Length > 5)
                        sDoc = Convert.ToInt64(dalCore.RetornaNumero( query.Cpf)).ToString("00000000000");
                    else {
                        if (!string.IsNullOrEmpty( query.Cnpj) && query.Cnpj.ToString().Length > 10)
                            sDoc = dalCore.RetornaNumero( query.Cnpj);
                        else
                            sDoc = "";
                        }
        ...
        public bool Existe_Notificacao_Passeio_Reparo(int Ano, int Numero) {
            using (GTI_Context db = new GTI_Context(_connection)) {
                var reg = (from i in db.Notificacao_Passeio_Reparo
                           where i.Ano_not == Ano && i.Numero_not == Numero select i.Inscricao).FirstOrDefault();
                if (string.IsNullOrEmpty(reg))
                    return false;
                else
        ...
        public List<Notificacao_Passeio_Reparo_Struct> Lista_Notificacao_Passeio_Reparo(int Ano) {
            using (GTI_Context db = new GTI_Context(_connection)) {
                var Sql = (from t in db.Notificacao_Passeio_Reparo
                           where t.Ano_not == Ano
                           orderby t.Numero_not select new {
                               Ano = t.Ano_not, Numero = t.Numero_not, Codigo = t.Codigo, Data_Cadastro = t.Data_cadastro, Usuario = t.Userid, Situacao = t.Situacao, Nome = t.Nome, Prazo = t.Prazo
                           }).ToList();
                List<Notificacao_Passeio_Reparo_Struct> Lista = new List<Notificacao_Passeio_Reparo_Struct>();
                foreach (var item in Sql) {
                    Notificacao_Passeio_Reparo_Struct reg = new Notificacao_Passeio_Reparo_Struct() {
                        Ano_Notificacao = item.Ano,
                        Numero_Notificacao = item.Numero,
                        Codigo_Imovel = item.Codigo,
                        Data_Cadastro = item.Data_Cadastro,
        ...
        public Notificacao_Passeio_Reparo_Struct Retorna_Notificacao_Passeio_Reparo(int Ano, int Numero) {
            using (GTI_Context db = new GTI_Context(_connection)) {
                ...
                Notificacao_Passeio_Reparo_Struct reg = null;
                if (Sql != null) {
                    reg = new Notificacao_Passeio_Reparo_Struct() {
                        Ano_Notificacao = Sql.Ano,
                        Numero_Notificacao = Sql.Numero,
                        AnoNumero = Sql.Numero.ToString("0000") + "/" + Sql.Ano.ToString(),
                        Codigo_Imovel = Sql.Codigo,
        ...
        public Exception Incluir_Notificacao_Passeio_Reparo(Notificacao_Passeio_Reparo Reg) {
            using (var db = new GTI_Context(_connection)) {
                ...
                db.Database.ExecuteSqlCommand("INSERT INTO notificacao_passeio_reparo(ano_not,numero_not,codigo,situacao,endereco_infracao,endereco_prop,endereco_entrega,nome,inscricao,prazo,data_cadastro," +
                                              "userid,nome2,codigo_cidadao,codigo_cidadao2,cpf,rg,cpf2,rg2,endereco_prop2,endereco_entrega2) " +
                                              " VALUES(@ano_not,@numero_not,@codigo,@situacao,@endereco_infracao,@endereco_prop,@endereco_entrega,@nome,@inscricao,@prazo,@data_cadastro,@userid," +
                                              "@nome2,@codigo_cidadao,@codigo_cidadao2,@cpf,@rg,@cpf2,@rg2,@endereco_prop2,@endereco_entrega2)", Parametros);
                try {
        ...
        public Exception Incluir_auto_infracao_Passeio_Reparo(Auto_infracao_passeio_reparo Reg) {
            using (var db = new GTI_Context(_connection)) {
                ...
                db.Database.ExecuteSqlCommand("INSERT INTO auto_infracao_passeio_reparo(ano_auto,numero_auto,ano_notificacao,numero_notificacao,data_notificacao,data_cadastro,userid) " +
                                              "VALUES(@ano_auto,@numero_auto,@ano_notificacao,@numero_notificacao,@data_notificacao,@data_cadastro,@userid)", Parametros);
                try {
                    db.SaveChanges();
                } catch (Exception ex) {
        ...
        public List<Auto_Infracao_Passeio_Reparo_Struct> Lista_Auto_Infracao_Passeio_Reparo(int Ano) {
            using (GTI_Context db = new GTI_Context(_connection)) {
                ...
                List<Auto_Infracao_Passeio_Reparo_Struct> Lista = new List<Auto_Infracao_Passeio_Reparo_Struct>();
                foreach (var item in Sql) {
                    Auto_Infracao_Passeio_Reparo_Struct reg = new Auto_Infracao_Passeio_Reparo_Struct() {
                        Ano_Auto = item.AnoAuto,
                        Numero_Auto = item.NumeroAuto,
                        Ano_Notificacao = item.AnoNot,
                        Numero_Notificacao = item.NumeroNot,
        ...
        public Auto_Infracao_Passeio_Reparo_Struct Retorna_Auto_Infracao_Passeio_Reparo(int Ano, int Numero) {
            using (GTI_Context db = new GTI_Context(_connection)) {
                ...
                Auto_Infracao_Passeio_Reparo_Struct reg = null;
                if (Sql != null) {
                    reg = new Auto_Infracao_Passeio_Reparo_Struct() {
                        Ano_Auto = Sql.AnoAuto,
                        Numero_Auto = Sql.NumeroAuto,
                        Ano_Notificacao = Sql.AnoNot,
                        Numero_Notificacao = Sql.NumeroNot,
        ...
        public bool Existe_Auto_Infracao_Passeio_Reparo(int Ano, int Numero) {
            using (GTI_Context db = new GTI_Context(_connection)) {
                var reg = (from i in db.Auto_Infracao_Passeio_Reparo
                           where i.Ano_auto == Ano && i.Numero_auto == Numero select i.Numero_notificacao).FirstOrDefault();
                if (reg == 0)
                    return false;
                else
