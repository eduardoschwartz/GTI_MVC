...
namespace GTI_Mvc.Controllers {
    public class NotificacaoController : Controller {
        ...
        [Route("Notificacao_pass_menu")]
        [HttpGet]
        public ActionResult Notificacao_pass_menu() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            return View();
        }

        [Route("Notificacao_pass_add")]
        [HttpGet]
        public ActionResult Notificacao_pass_add() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            NotificacaoPassViewModel model = new NotificacaoPassViewModel();
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Notificacao_pass_add(NotificacaoPassViewModel model, string action) {
            if (model.Codigo_Imovel == 0) {
                ViewBag.Result = "Código de imóvel inválido.";
                return View(model);
            }
            ...
            if (action == "btnCodigoOK") {
                ...
                if (Listaprop.Count == 0) {
                    ViewBag.Result = "Não é possível emitir notificação para este imóvel.";
                    model = new NotificacaoPassViewModel();
                    return View(model);
                }
                foreach (ProprietarioStruct prop in Listaprop) {
                    if (prop.Tipo == "P" && prop.Principal) {
            ...
            if (action == "btnValida") {
                bool _existe = imovelRepository.Existe_Notificacao_Passeio(model.Ano_Notificacao, model.Numero_Notificacao);
                if (_existe) {
                    ViewBag.Result = "Nº de notificação já cadastrado.";
                    return View(model);
                } else {
                    ...
                    Save_Notificacao_Passeio(model);
        ...
        private ActionResult Save_Notificacao_Passeio(NotificacaoPassViewModel model) {
            Notificacao_Passeio reg = new Notificacao_Passeio() {
                Ano_not = model.Ano_Notificacao,
                Numero_not = model.Numero_Notificacao,
                Codigo = model.Codigo_Imovel,
                Inscricao = model.Inscricao,
        ...
        [Route("Notificacao_pass_query")]
        [HttpGet]
        public ActionResult Notificacao_pass_query() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
            ...
            ViewBag.Lista_Ano = new SelectList(Lista_Ano);
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Notificacao_Passeio_Struct> _listaNot = imovelRepository.Lista_Notificacao_Passeio(DateTime.Now.Year);
            foreach (Notificacao_Passeio_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Notificacao = item.Ano_Notificacao,
                    Numero_Notificacao = item.Numero_Notificacao,
                    Codigo_Imovel = item.Codigo_Imovel,
            ...
            NotificacaoPassQueryViewModel model = new NotificacaoPassQueryViewModel();
            model.ListaNotificacao = ListaNot;
            model.Ano_Selected = DateTime.Now.Year;
            return View(model);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Notificacao_pass_query(NotificacaoPassQueryViewModel model2) {
            int _ano = model2.Ano_Selected;
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Notificacao_Passeio_Struct> _listaNot = imovelRepository.Lista_Notificacao_Passeio(_ano);
            foreach (Notificacao_Passeio_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Notificacao = item.Ano_Notificacao,
        ...
        public ActionResult Notificacao_passeio_print(int a, int n) {
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            Notificacao_Passeio_Struct _not = imovelRepository.Retorna_Notificacao_Passeio(a, n);
            ...
            ReportDocument rd = new ReportDocument();
            rd.Load(System.Web.HttpContext.Current.Server.MapPath("~/Reports/Notificacao_Passeio.rpt"));

            try {
                rd.SetDataSource(ListaNot);
                Stream stream = rd.ExportToStream(ExportFormatType.PortableDocFormat);
                return File(stream, "application/pdf", "Notificacao_Passeio.pdf");
            } catch {
        ...
        [Route("AutoInfracao_pass_add")]
        [HttpGet]
        public ActionResult AutoInfracao_pass_add() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            NotificacaoPassViewModel model = new NotificacaoPassViewModel();
            List<int> Lista_Ano = new List<int>();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AutoInfracao_pass_add(NotificacaoPassViewModel model, string action) {
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            }
            ...
            if (action == "btnCodigoOK") {
                ...
                bool _existe = imovelRepository.Existe_Notificacao_Passeio(_ano, _num);
                if (!_existe) {
                    ViewBag.Result = "Notificação não cadastrada.";
                    return View(model);
                }
                Notificacao_Passeio_Struct _dados = imovelRepository.Retorna_Notificacao_Passeio(_ano, _num);
                model.Codigo_cidadao = _dados.Codigo_cidadao;
                model.Codigo_cidadao2 = _dados.Codigo_cidadao2;
                model.Nome_Proprietario = _dados.Nome_Proprietario;
                model.Nome_Proprietario2 = _dados.Nome_Proprietario2;
            ...
            if (action == "btnValida") {
                ...
                bool _existe = imovelRepository.Existe_Auto_Infracao_Passeio(_ano_auto, _num_auto);
                if (_existe) {
                    ViewBag.Result = "Auto de infração já cadastrado.";
                    return View(model);
                } else {
        ...
        [Route("AutoInfracao_pass_query")]
        [HttpGet]
        public ActionResult AutoInfracao_pass_query() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Auto_Infracao_Passeio_Struct> _listaNot = imovelRepository.Lista_Auto_Infracao_Passeio(DateTime.Now.Year);
            foreach (Auto_Infracao_Passeio_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Auto = item.Ano_Auto,
                    Numero_Auto = item.Numero_Auto,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AutoInfracao_pass_query(NotificacaoPassQueryViewModel model2) {
            int _ano = model2.Ano_Selected;
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Auto_Infracao_Passeio_Struct> _listaNot = imovelRepository.Lista_Auto_Infracao_Passeio(_ano);
            foreach (Auto_Infracao_Passeio_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Auto = item.Ano_Auto,
        ...
        public ActionResult AutoInfracao_passeio_print(int a, int n) {
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            Auto_Infracao_Passeio_Struct _not = imovelRepository.Retorna_Auto_Infracao_Passeio(a, n);
            ...
            ReportDocument rd = new ReportDocument();
            rd.Load(System.Web.HttpContext.Current.Server.MapPath("~/Reports/Auto_Infracao_Passeio.rpt"));

            try {
                rd.SetDataSource(ListaNot);
                Stream stream = rd.ExportToStream(ExportFormatType.PortableDocFormat);
                return File(stream, "application/pdf", "Auto_Infracao_Passeio.pdf");
            } catch {
        ...
        [Route("Notificacao_pass_reparo_menu")]
        [HttpGet]
        public ActionResult Notificacao_pass_reparo_menu() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            return View();
        }

        [Route("Notificacao_pass_reparo_add")]
        [HttpGet]
        public ActionResult Notificacao_pass_reparo_add() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            NotificacaoPassViewModel model = new NotificacaoPassViewModel();
            List<int> Lista_Ano = new List<int>();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Notificacao_pass_reparo_add(NotificacaoPassViewModel model, string action) {
            if (model.Codigo_Imovel == 0) {
                ViewBag.Result = "Código de imóvel inválido.";
                return View(model);
            }
            ...
            if (action == "btnValida") {
                bool _existe = imovelRepository.Existe_Notificacao_Passeio_Reparo(model.Ano_Notificacao, model.Numero_Notificacao);
                if (_existe) {
                    ViewBag.Result = "Nº de notificação já cadastrado.";
                    return View(model);
                } else {
        ...
        private ActionResult Save_Notificacao_Passeio_Reparo(NotificacaoPassViewModel model) {
            Notificacao_Passeio_Reparo reg = new Notificacao_Passeio_Reparo() {
                Ano_not = model.Ano_Notificacao,
                Numero_not = model.Numero_Notificacao,
                Codigo = model.Codigo_Imovel,
                Inscricao = model.Inscricao,
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            Exception ex = imovelRepository.Incluir_Notificacao_Passeio_Reparo(reg);
            return null;

        }

        [Route("Notificacao_pass_reparo_query")]
        [HttpGet]
        public ActionResult Notificacao_pass_reparo_query() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Notificacao_Passeio_Reparo_Struct> _listaNot = imovelRepository.Lista_Notificacao_Passeio_Reparo(DateTime.Now.Year);
            foreach (Notificacao_Passeio_Reparo_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Notificacao = item.Ano_Notificacao,
                    Numero_Notificacao = item.Numero_Notificacao,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Notificacao_pass_reparo_query(NotificacaoPassQueryViewModel model2) {
            int _ano = model2.Ano_Selected;
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Notificacao_Passeio_Reparo_Struct> _listaNot = imovelRepository.Lista_Notificacao_Passeio_Reparo(_ano);
            foreach (Notificacao_Passeio_Reparo_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Notificacao = item.Ano_Notificacao,
        ...
        public ActionResult Notificacao_passeio_reparo_print(int a, int n) {
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            Notificacao_Passeio_Reparo_Struct _not = imovelRepository.Retorna_Notificacao_Passeio_Reparo(a, n);
            ...
            ReportDocument rd = new ReportDocument();
            rd.Load(System.Web.HttpContext.Current.Server.MapPath("~/Reports/Notificacao_Passeio_Reparo.rpt"));

            try {
                rd.SetDataSource(ListaNot);
                Stream stream = rd.ExportToStream(ExportFormatType.PortableDocFormat);
                return File(stream, "application/pdf", "Notificacao_Passeio_Reparo.pdf");
            } catch {
        ...
        [Route("AutoInfracao_pass_reparo_add")]
        [HttpGet]
        public ActionResult AutoInfracao_pass_reparo_add() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            NotificacaoPassViewModel model = new NotificacaoPassViewModel();
            List<int> Lista_Ano = new List<int>();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AutoInfracao_pass_reparo_add(NotificacaoPassViewModel model, string action) {
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            }
            ...
            if (action == "btnCodigoOK") {
                ...
                bool _existe = imovelRepository.Existe_Notificacao_Passeio_Reparo(_ano, _num);
                if (!_existe) {
                    ViewBag.Result = "Notificação não cadastrada.";
                    return View(model);
                }
                Notificacao_Passeio_Reparo_Struct _dados = imovelRepository.Retorna_Notificacao_Passeio_Reparo(_ano, _num);
                model.Codigo_cidadao = _dados.Codigo_cidadao;
                model.Codigo_cidadao2 = _dados.Codigo_cidadao2;
                model.Nome_Proprietario = _dados.Nome_Proprietario;
                model.Nome_Proprietario2 = _dados.Nome_Proprietario2;
            ...
            if (action == "btnValida") {
                ...
                bool _existe = imovelRepository.Existe_Auto_Infracao_Passeio_Reparo(_ano_auto, _num_auto);
                if (_existe) {
                    ViewBag.Result = "Auto de infração já cadastrado.";
                    return View(model);
                } else {
        ...
        private ActionResult Save_Auto_Infracao_Passeio_Reparo(NotificacaoPassViewModel model) {
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            Exception ex = imovelRepository.Incluir_auto_infracao_Passeio_Reparo(reg);
            return null;
        }

        [Route("AutoInfracao_pass_reparo_query")]
        [HttpGet]
        public ActionResult AutoInfracao_pass_reparo_query() {
            if (Session["hashid"] == null)
                return RedirectToAction("Login", "Home");
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Auto_Infracao_Passeio_Reparo_Struct> _listaNot = imovelRepository.Lista_Auto_Infracao_Passeio_Reparo(DateTime.Now.Year);
            foreach (Auto_Infracao_Passeio_Reparo_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Auto = item.Ano_Auto,
                    Numero_Auto = item.Numero_Auto,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AutoInfracao_pass_reparo_query(NotificacaoPassQueryViewModel model2) {
            int _ano = model2.Ano_Selected;
            List<int> Lista_Ano = new List<int>();
            for (int i = 2020; i <= DateTime.Now.Year; i++) {
                Lista_Ano.Add(i);
            ...
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            List<NotificacaoPassViewModel> ListaNot = new List<NotificacaoPassViewModel>();
            List<Auto_Infracao_Passeio_Reparo_Struct> _listaNot = imovelRepository.Lista_Auto_Infracao_Passeio_Reparo(_ano);
            foreach (Auto_Infracao_Passeio_Reparo_Struct item in _listaNot) {
                NotificacaoPassViewModel reg = new NotificacaoPassViewModel() {
                    AnoNumero = item.AnoNumero,
                    Ano_Auto = item.Ano_Auto,
        ...
        public ActionResult AutoInfracao_passeio_reparo_print(int a, int n) {
            Imovel_bll imovelRepository = new Imovel_bll(_connection);
            Auto_Infracao_Passeio_Reparo_Struct _not = imovelRepository.Retorna_Auto_Infracao_Passeio_Reparo(a, n);
            ...
            ReportDocument rd = new ReportDocument();
            rd.Load(System.Web.HttpContext.Current.Server.MapPath("~/Reports/Auto_Infracao_Passeio_Reparo.rpt"));

            try {
                rd.SetDataSource(ListaNot);
                Stream stream = rd.ExportToStream(ExportFormatType.PortableDocFormat);
                return File(stream, "application/pdf", "Auto_Infracao_Passeio_Reparo.pdf");
            } catch {
